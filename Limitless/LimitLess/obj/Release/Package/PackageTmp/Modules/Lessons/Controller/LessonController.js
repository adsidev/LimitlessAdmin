...
app.controller("LessonController", function ($scope, $http, $rootScope, $location,$cookieStore) {
    var reqdata = $.param({
        PageIndex: 1,
        PageSize: 10,
    ...
    $scope.DeleteLesson = function (e) {
        id = $(e.target).data('id');
        var req_data = $.param({
            ID: id
        });
        if (confirm('Are you sure you want to delete record?')) {
            $http.post("api/Lesson/DeleteLesson", req_data, {
                headers: { 'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8' }
            }).success(function (data) {
                toastr["success"]("Deleted Successfully.", 'Delete Lesson');
